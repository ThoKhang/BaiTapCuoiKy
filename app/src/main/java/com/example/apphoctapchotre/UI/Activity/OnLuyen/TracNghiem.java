...
public class TracNghiem extends AppCompatActivity {
    ...
    private void hienTatCaDapAn(boolean show) {
        int v = show ? View.VISIBLE : View.GONE;
        btnA.setVisibility(v);
        btnB.setVisibility(v);
        btnC.setVisibility(v);
        btnD.setVisibility(v);
    }
    ...
    private void checkAnswer(RadioButton btn, int index) {
        ...
        if (dung) demCauDung++; else demCauSai++;
    ...
    private void hoanThanh() {
        ...
        tienTrinh.setDaHoanThanh(demCauDung == demCauSai + demCauDung ? 1 : 0);
        taoTienTrinh();
    }
